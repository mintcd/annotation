...
function debugLog(...args: any[]) {
  try {
    const on = (window as any).__proxy_debug;
    if (!on) return;
    // store a brief log array on window for later inspection
    const arr = (window as any).__proxy_script_logs = (window as any).__proxy_script_logs || [];
    arr.push({ t: Date.now(), args });
    // also print to console to make it visible during development
    // eslint-disable-next-line no-console
    console.debug('[ScriptManager]', ...args);
  } catch { }
}
...
class ScriptManagerClass {
  ...
  loadScripts(pageUrl: string | undefined, scripts: ScriptItem[], opts?: { apiBase?: string; containerSelector?: string }) {
    ...
    debugLog('loadScripts called', { pageUrl: pageUrl || null, count: scripts.length });
      ...
      try {
        if (s.src) {
          ...
          if (existing) {
            ...
            debugLog('skip append - existing script found', { id, src: s.src });
          ...
          try {
            ...
            debugLog('appended external script', { id, src: s.src, parent: parent.tagName || 'unknown' });
          } catch (err) {
            try { (head || body || document.documentElement).appendChild(el); debugLog('appended external script fallback', { id, src: s.src }); } catch { debugLog('failed to append script', { id, src: s.src, err }); }
          }
          nodes.push(el);
        } else if (s.content) {
          const el = document.createElement('script');
          ...
          try {
            parent.appendChild(el);
            debugLog('appended inline script', { id, parent: parent.tagName || 'unknown' });
          } catch (err) {
            try { (body || head || document.documentElement).appendChild(el); debugLog('appended inline script fallback', { id }); } catch { debugLog('failed to append inline script', { id, err }); }
          }
          nodes.push(el);
        ...
        debugLog('error appending script', { id, err: String(e) });
      }
  ...
  markExecuted(pageKeyStr: string, id: string) {
    ...
    debugLog('markExecuted', { pageKey: pageKeyStr, id });
    emitProxySignal(id);
